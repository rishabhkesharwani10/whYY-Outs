          ...
          {userOrders.length > 0 ? (
            <div className="space-y-6">
              {userOrders.map(order => (
                <div key={order.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 border border-brand-gold/20 rounded-lg">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    ...
                    <ReactRouterDOM.Link to={`/order/${order.id}`} className="font-sans text-sm tracking-widest px-6 py-2 border border-brand-gold text-brand-gold hover:bg-brand-gold hover:text-brand-dark transition-colors duration-300 uppercase">
                        View Details
                    </ReactRouterDOM.Link>
                  </div>
                </div>
