...
const AdminEditProductPage: React.FC = () => {
    ...
    // This effect fetches the base product data from the DB
    useEffect(() => {
        const fetchProduct = async () => {
            ...
            if (fetchErr) {
                console.error("Error fetching product:", fetchErr.message || fetchErr);
    ...
    // This effect populates the form from sessionStorage if available, otherwise from the fetched product
    useEffect(() => {
        if (!product) return;
    ...
    useEffect(() => {
        if (product && !loading) { // Only save when not loading and product exists
    ...
    }, [formData, featuresInput, sizesInput, product, loading]);
    ...
    if (error && !product) { return <AdminLayout><div className="text-center p-8"><p className="text-red-500">{error}</p><BackButton fallback="/admin/products" /></div></AdminLayout>; }
    if (!product) { return <AdminLayout><p className="text-center p-8">Product not found.</p></AdminLayout>; }
