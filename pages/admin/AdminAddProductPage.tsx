...
import { NAVIGATION_CATEGORIES, ADMIN_USER_ID } from '../../constants.ts';
...
const AdminAddProductPage: React.FC = () => {
  ...
  const initialFormData = {
    ...
    returnDays: undefined, warrantyDetails: '', features: [], sizes: [],
  };

  const [formData, setFormData] = useState<Partial<Product>>(() => getInitialState('adminAddProductFormData', initialFormData));
  const [featuresInput, setFeaturesInput] = useState<string>(() => sessionStorage.getItem('adminAddProductFeaturesInput') || '');
  const [sizesInput, setSizesInput] = useState<string>(() => sessionStorage.getItem('adminAddProductsizesInput') || '');
  ...
  useEffect(() => { sessionStorage.setItem('adminAddProductFormData', JSON.stringify(formData)); }, [formData]);
  useEffect(() => { sessionStorage.setItem('adminAddProductFeaturesInput', featuresInput); }, [featuresInput]);
  useEffect(() => { sessionStorage.setItem('adminAddProductsizesInput', sizesInput); }, [sizesInput]);
  ...
  const clearSavedState = () => {
    sessionStorage.removeItem('adminAddProductFormData');
    sessionStorage.removeItem('adminAddProductFeaturesInput');
    sessionStorage.removeItem('adminAddProductsizesInput');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const productData: Omit<Product, 'id' | 'rating' | 'reviewCount'> = {
        ...
        sellerId: ADMIN_USER_ID, // Assign product to the platform
    ...
    <AdminLayout>
      <div className="page-fade-in">
        ...
        <div className="max-w-4xl mx-auto bg-black/30 border border-brand-gold/20 rounded-lg shadow-lg p-8">
          <h1 className="font-serif text-4xl text-brand-light mb-2">Add Global Product</h1>
          <p className="text-brand-light/70 mb-8">Create a new product for the main platform store.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <FormSection title="Attributes & Features">
                ...
                <div><label htmlFor="featuresInput" className={formLabelClass}>Features (One per line)</label><textarea id="featuresInput" name="featuresInput" value={featuresInput} onChange={e => setFeaturesInput(e.target.value)} rows={4} className={formInputClass} /></div>
            </FormSection>
            ...
            <div className="pt-4">
              ...
              >
                {isUploading ? (<><div className="w-5 h-5 border-2 border-white/40 border-t-white rounded-full animate-spin mr-3"></div><span>{uploadStatus || 'Processing...'}</span></>) : ('Add Global Product')}
              </button>
            </div>
          </form>
        </div>
