...
const CartPage: React.FC = () => {
  const { cartItems, removeFromCart, updateQuantity, itemCount, subtotal, gstAmount, platformFee, grandTotal, applyCoupon, removeCoupon, couponCode, couponDiscount, couponMessage } = useCart();
          ...
          <div className="lg:col-span-2 bg-black/20 border border-brand-gold/20 rounded-lg shadow-lg shadow-brand-gold/10 p-6">
            ...
            <div className="space-y-6">
              {cartItems.map(item => (
                <div 
                  key={item.cartItemId} 
                  className={`flex flex-col sm:flex-row items-center justify-between border-b border-brand-gold/20 pb-6 last:border-b-0 transition-all duration-300 ease-out ${removingItemId === item.cartItemId ? 'opacity-0 -translate-x-full max-h-0 py-0 border-none' : 'max-h-96'}`}
                ...
                >
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="flex items-center border border-brand-gold/50 rounded-md">
                      <button onClick={() => handleUpdateQuantity(item.cartItemId, item.quantity - 1)} className="p-2 text-brand-gold hover:bg-brand-gold/10"><Icon name="minus" className="w-4 h-4" /></button>
                      <span className="px-4 font-semibold">{item.quantity}</span>
                      <button onClick={() => handleUpdateQuantity(item.cartItemId, item.quantity + 1)} className="p-2 text-brand-gold hover:bg-brand-gold/10"><Icon name="plus" className="w-4 h-4" /></button>
                    </div>
                    ...
                    <button onClick={() => handleRemove(item.cartItemId)} className="text-brand-light/70 hover:text-red-500 transition-colors"><Icon name="trash" className="w-5 h-5"/></button>
                  </div>
                </div>
              ))}
            </div>
