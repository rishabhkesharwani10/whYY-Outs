...
const AboutUsPage: React.FC = () => {
    ...
    const teamMembers = [
        { name: 'Rohan Verma', title: 'Founder & CEO', avatar: 'https://picsum.photos/seed/rohan/200', bio: 'Visionary leader with a passion for blending technology and luxury retail to create unparalleled customer experiences.' },
        { name: 'Priya Sharma', title: 'Head of Product', avatar: 'https://picsum.photos/seed/priya/200', bio: 'Expert in user-centric design, dedicated to building an intuitive and delightful shopping journey on whYYOuts.' },
        { name: 'Arjun Singh', title: 'Lead AI Engineer', avatar: 'https://picsum.photos/seed/arjun/200', bio: 'The mastermind behind our AI Co-Pilot and DealBrain™, pushing the boundaries of personalized e-commerce.' },
    ];
    ...
    return (
        <LegalPageLayout title="About whYYOuts">
            ...
            <div className="mt-12 text-left">
                ...
                <H2>Meet The Team</H2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-6 text-center">
                    {teamMembers.map(member => (
                        <div key={member.name}>
                            <img src={member.avatar} alt={member.name} className="w-32 h-32 rounded-full object-cover mx-auto border-2 border-brand-gold/50 shadow-lg"/>
                            <h4 className="font-serif text-xl text-brand-light mt-4">{member.name}</h4>
                            <p className="text-brand-gold text-sm">{member.title}</p>
                            <p className="text-xs text-brand-light/70 mt-2">{member.bio}</p>
                        </div>
                    ))}
                </div>
